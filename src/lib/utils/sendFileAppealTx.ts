...
export type SendFileAppealTxParams = {
  onchainJobId: bigint;
  account?: Address;
};
...
export async function sendFileAppealTx({
  onchainJobId,
  account: connectedHint,
}: SendFileAppealTxParams): Promise<`0x${string}`> {
  ...
  if (
    ...
    console.warn('[fileAppeal] wagmi account hint mismatch', { connectedHint, signingAccount });
  }
  ...
  try {
    ...
    logContractError('fileAppeal simulateContract', simErr);
  ...
  try {
    ...
    logContractError('fileAppeal sendTransaction', err);
    throw new Error(decodeContractError(err, contracts.escrowVault.abi as Abi, 'fileAppeal'));
  }
}
